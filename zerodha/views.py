...
class MarketOrderBuy(APIView):
    def post(self, request):
        ...
        try:
            ...
            try:
                order_id = market_buy_order(kite_, data['trading_symbol'], exchange, data['quantity'])
                return Response({
...
class MarketOrderSell(APIView):
    def post(self, request):
        ...
        try:
            ...
            try:
                order_id = market_sell_order(kite_, data['trading_symbol'], exchange, data['quantity'])
                return Response({
                    'message': f'order id is {order_id}'
                }, status=status.HTTP_200_OK)
            except Exception as ex: 
...
class LimitOrderBuy(APIView):
    def post(self, request):
        ...
        try:
            ...
            try:
                order_id = limit_buy_order(kite_, data['trading_symbol'], exchange, data['quantity'], data['price'])
                return Response({
                    'message': f'order id is {order_id}'
                }, status=status.HTTP_200_OK)
            except Exception as ex:
...
class LimitOrderSell(APIView):
    def post(self, request):
        ...
        try:
            ...
            try:
                order_id = limit_sell_order(kite_, data['trading_symbol'], exchange, data['quantity'], data['price'])
                return Response({
                    'message': f'order id is {order_id}'
                }, status=status.HTTP_200_OK)
            except Exception as ex: 
